...
function loadPrivateKeys() {
  try {
    const data = fs.readFileSync("nisa.txt", "utf8");
    privateKeys = data.split("\n").map(key => key.trim()).filter(key => key.match(/^(0x)?[0-9a-fA-F]{64}$/));
    if (privateKeys.length === 0) throw new Error("No valid private keys in nisa.txt");
    addLog(`Loaded ${privateKeys.length} private keys from nisa.txt`, "success");
  } catch (error) {
...
async function bridge(wallet, amount, recipient, destChainId) {
  try {
    ...
    addLog(`Debug: Building bridge transaction for amount ${amount} Hellios to ${getShortAddress(wallet.address)}`, "debug");
    ...
    if (allowance < amountWei) {
      addLog(`Approving router to spend ${amount} Hellios`, "info");
...
async function stake(wallet, amount, validatorAddress, validatorName) {
  try {
    ...
    addLog(`Debug: Building stake transaction for amount ${amount} Hellios to validator ${validatorName || validatorAddress}`, "debug");
    ...
    for (let accountIndex = 0; accountIndex < privateKeys.length && !shouldStop; accountIndex++) {
      ...
      for (let bridgeCount = 0; bridgeCount < dailyActivityConfig.bridgeRepetitions && !shouldStop; bridgeCount++) {
        ...
        try {
          ...
          const hlsBalance = await tokenContract.balanceOf(wallet.address);
          addLog(`Account ${accountIndex + 1} - Bridge ${bridgeCount + 1}: Hellios Balance: ${ethers.formatUnits(hlsBalance, 18)}`, "wait");
          addLog(`Account ${accountIndex + 1} - Bridge ${bridgeCount + 1}: Bridge ${amountHLS} Hellios Hellios ➯  ${destChainName}`, "info");
          ...
          if (nativeBalance < gasCost) {
            addLog(`Account ${accountIndex + 1} - Bridge ${bridgeCount + 1}: Insufficient native balance (${ethers.formatEther(nativeBalance)} Hellios)`, "error");
            continue;
          }
          if (hlsBalance < amountWei) {
            addLog(`Account ${accountIndex + 1} - Bridge ${bridgeCount + 1}: Insufficient Hellios balance (${ethers.formatUnits(hlsBalance, 18)} Hellios)`, "error");
            continue;
          }
      ...
      for (let stakeCount = 0; stakeCount < dailyActivityConfig.stakeRepetitions && !shouldStop; stakeCount++) {
        ...
        try {
          addLog(`Account ${accountIndex + 1} - Stake ${stakeCount + 1}: Stake ${amountHLS} Hellios to ${validator.name}`, "info");
...
const dailyActivitySubMenu = blessed.list({
  ...
  items: [
    ...
    "Set Hellios Range For Bridge",
    "Set Stake Repetitions",
    "Set Hellios Range For Stake",
    "Back to Main Menu"
  ],
  padding: { left: 1, top: 1 },
  hidden: true
    ...
    if (!isHeaderRendered) {
      figlet.text("ANNISA".toUpperCase(), { font: "ANSI Shadow" }, (err, data) => {
        ...
        try {
          ...
          addLog(`UI render error: ${e.message}`, "error");
        }
        renderQueue.shift();
        isRendering = false;
...
function updateStatus() {
  try {
    ...
    const status = activityRunning
      ...
      : chalk.green("Idle");
    const statusText = `Status: ${status} | Active Account: ${getShortAddress(walletInfo.address)} | Total Accounts: ${privateKeys.length} | Auto Bridge: ${dailyActivityConfig.bridgeRepetitions}x | Auto Stake: ${dailyActivityConfig.stakeRepetitions}x | HELIOS TESTNET`;
...
async function updateWallets() {
  try {
    ...
    const header = `${chalk.bold.cyan("     Address").padEnd(12)}                   ${chalk.bold.cyan("Hellios".padEnd(8))}`;
...
dailyActivitySubMenu.on("select", (item) => {
  ...
  switch (action) {
    case "Set Bridge Repetitions":
      ...
      break;
    case "Set Hellios Range For Bridge":
      configForm.configType = "hlsRangeBridge";
      configForm.setLabel(" Enter Hellios Range for Bridge ");
      minLabel.show();
      maxLabel.show();
    ...
    case "Set Stake Repetitions":
      ...
      break;
    case "Set Hellios Range For Stake":
      configForm.configType = "hlsRangeStake";
      configForm.setLabel(" Enter Hellios Range for Stake ");
      minLabel.show();
      maxLabel.show();
...
configForm.on("submit", () => {
  ...
  try {
    ...
    if (configForm.configType === "hlsRangeBridge" || configForm.configType === "hlsRangeStake") {
      ...
      if (isNaN(maxValue) || maxValue <= 0) {
        addLog("Invalid Max Hellios value. Please enter a positive number.", "error");
        configInputMax.setValue("");
  ...
  if (configForm.configType === "bridgeRepetitions") {
    ...
    if (value > maxValue) {
      addLog("Min Hellios cannot be greater than Max Hellios.", "error");
      configInput.setValue("");
      configInputMax.setValue("");
      screen.focusPush(configInput);
      safeRender();
    ...
    addLog(`Hellios Range for Bridge set to ${dailyActivityConfig.minHlsBridge} - ${dailyActivityConfig.maxHlsBridge}`, "success");
    ...
    addLog(`Hellios Range for Stake set to ${dailyActivityConfig.minHlsStake} - ${dailyActivityConfig.maxHlsStake}`, "success");
  }
  saveConfig();
  updateStatus();
